                ...
                <tbody className="divide-y divide-white/5">
                    {futureStats.map((s, idx) => (
                        <tr key={s.month} className="hover:bg-white/[0.02] transition-colors group">
                            <td className="px-5 py-4 border-r border-white/5">
                                <span className="text-sm font-black text-slate-300">
                                    {new Date(s.month + '-01').toLocaleDateString(undefined, { month: 'short', year: '2-digit' })}
                                </span>
                            </td>
                            <td className="px-5 py-4 text-sm font-black text-blue-400 text-right tabular-nums bg-blue-500/[0.02]">
                                ₹{s.safeSpendLimit.toFixed(1)} <span className="text-[10px] opacity-60 ml-0.5">CR</span>
                            </td>
                            <td className="px-5 py-4 text-sm font-bold text-slate-400 text-right tabular-nums">
                                ₹{s.plannedOutflowTotal.toFixed(1)} <span className="text-[10px] opacity-60 ml-0.5">CR</span>
                            </td>
                            <td className={clsx(
                                "px-5 py-4 text-sm font-black text-right tabular-nums",
                            ...
                            <td className="px-5 py-4">
                                <div className="flex justify-center">
                                    {s.gapToFix > 5 ? (
                                        <AlertTriangle size={18} className="text-rose-500 shadow-lg shadow-rose-900/20" />
                                    ) : s.gapToFix > 0 ? (
                    ...
                    <tr className="bg-blue-600/5 border-t-2 border-blue-500/20">
                        ...
                        <td colSpan={2} className="px-5 py-4 text-[11px] font-bold text-slate-500 italic">
                            Align engineering start-dates to match P20 inflow capacity to avoid stoppage risk.
                        </td>
                        <td className="px-5 py-4 text-sm font-black text-right text-blue-400 tabular-nums">
                        ...
                        <td className="px-5 py-4">
                            <div className="flex justify-center px-3 py-1.5 bg-blue-600 rounded text-[10px] font-black text-white uppercase cursor-pointer hover:bg-blue-500 transition-colors">
                                Apply
                            </div>
                        </td>
                    </tr>
